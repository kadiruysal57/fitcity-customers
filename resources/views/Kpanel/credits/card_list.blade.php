...
@section('content')
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12 col-md-12">
                                ...
                                @endif
                                @if(!empty($credits) && empty($credits->save_card->status))
                                    <div class="row">
                                        @foreach($credits->save_card as $save)

                                            <div class="credit-card {{$save->schema}} selectable col-md-4 col-12 {{$save->ctoken}}">
                                                <button type="button" class="btn btn-danger btn-sm card-delete-button" data-ctoken="{{$save->ctoken}}">
                                ...
                                @else
                                    <center><h3>Kayıtlı Kartınız Mevcut Değil</h3></center>
                                @endif
                                <button type="button" class="btn btn-success" data-bs-toggle="modal"
                                        data-bs-target="#add_credit_card">
                                    <i class="fa fa-credit-card"></i> Kart Ekle
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="add_credit_card" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="cc_owner">Kart Sahibi Adı</label>
                        <input type="text" class="form-control" id="cc_owner" name="cc_owner">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="card_number">Son Kullanma Tarihi</label>
                        <div class="row">
                            <div class="col-6">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" id="addCard" class="btn btn-success">Kaydet</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function () {
            $('.cancel_modal').click(function(){
                $('#add_credit_card').modal('hide');
            })
            $('.card-delete-button').click(function(){
                ctoken = $(this).attr('data-ctoken');
                Swal.fire({
                    title: 'Kart Silinecek Emin Misiniz?',
                    icon: 'warning',
                ...
                if (cc_owner === '' || card_number === '' || expiry_month === '' || expiry_year === '' || cvv === '') {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $('.add_credit_card').modal('hide');
                            $('#sanal_dcard_modal').html(response.message);
